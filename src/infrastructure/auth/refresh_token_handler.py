...
재사용 감지·방어적 폐기는 요청을 거부(raise)하기 전에 커밋해야 한다. 그렇지 않으면
get_async_session의 예외 롤백에 폐기 UPDATE가 함께 사라진다. 이 '거부 직전 영속화'는
happy path의 커밋과 구분되도록 persist_before_reject로만 수행한다(정상 회전의 커밋은
세션을 연 호출부 의존성이 담당한다 — 커밋 책임 단일 규칙은 db.session 참조).
"""
