...
# 위상 깊이 하드캡. 처음 2로 잡았던 건 당시 실측이 전부 "별 모양"(레벨 0→1)이어서였는데,
# 장 간 데이터 전달을 되살리면서(2026-08-20 사용자 결정) 사슬이 생겼다 —
# 1.1→4.1(비교 기준점)→4.5(4.* 종합) = 레벨 3. 3이면 "앞 장 승계 → 장 내 종합"까지
# 담고, 그 이상은 순차 배치만 길어져 병렬 손실이 커진다. 초과분은 절단+경고(막지 않음).
MAX_DEPTH = 3
