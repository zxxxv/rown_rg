...
class Settings(BaseSettings):
    ...
    # 다국어 임베딩으로 다시 본다. **기본 off**: 지금은 뷰 시점에 계산해 절당 20~30초가
    # 걸린다(2026-08-26 실측). 대목 벡터를 색인 시점에 저장하는 이관이 끝나면 켠다 —
    # 그때는 "주장 N개 임베딩 + 행렬곱"이라 밀리초 단위다.
    dense_align_enabled: bool = False
