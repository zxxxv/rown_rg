...
# 어휘는 v6 병기 대조 실측(2026-08-28)으로 보강 — "넘어 추가성"·"따라 탄소정보공개
# 프로젝트"·"이른바 리셔플링"류 문맥 조각이 용어로 잡혀 요동 오탐을 만들었다.
_KO_FRAGMENT_RE = re.compile(
    r"(?:[을를이가은는와과에의로]|으로|에서|하는|되는|같은|또는|동시에|위한|따른"
    r"|넘어|따라|대해|대한|통한|관한|인한|아니라|이라도|이른바|불리는|비롯한|아닌"
    r"|중인|위해|및|년까지|년부터)\s"
)
